...
local lock_command = string.format("i3lock -i %s/.config/awesome/themes/lock.png", os.getenv("HOME"))
...
awful.screen.connect_for_each_screen(function(s)
    ...
    -- Floating wibox
    s.mywibox = awful.wibar({ position = "top", screen = s, border_width = 5, height = 26, shape = gears.shape.rounded_rect, bg = beautiful.bg_normal .. "55" })
...
local globalkeys = gears.table.join(
    ...
    awful.key({ modkey, "Shift" }, "l", function () awful.spawn(lock_command) end,
...
local clientkeys = gears.table.join(
    ...
    awful.key({ }, "Print", function ()
        awful.util.spawn("mate-screenshot", false)
    end)
)
